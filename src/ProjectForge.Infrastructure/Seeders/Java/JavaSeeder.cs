...
public static class JavaSeeder
{
    ...
    public static IEnumerable<DesignPatternEntry> GetDesignPatterns() => new[]
    {
        ...
        new DesignPatternEntry { Id=1102, CreatedAt=SeedDate, Pattern=DesignPattern.Mediator, Name="Mediator (Spring Boot)", Architecture=ArchitectureType.Java, Description="Patrón Mediator con ApplicationEventPublisher o Axon.", ImplementationNotes="Usar ApplicationEventPublisher para desacoplar componentes. Handlers anotados con @EventListener.", ScaffoldCommandsJson="[\"mkdir -p src/main/java/application/mediator\"]" },
        new DesignPatternEntry { Id=1103, CreatedAt=SeedDate, Pattern=DesignPattern.Saga, Name="Saga (Spring Boot)", Architecture=ArchitectureType.Java, Description="Orquesta transacciones distribuidas con compensación.", ImplementationNotes="Implementar saga de orquestación con @Service. Cada paso compensa los anteriores en caso de fallo.", ScaffoldCommandsJson="[\"mkdir -p src/main/java/application/saga\"]" },
        new DesignPatternEntry { Id=1104, CreatedAt=SeedDate, Pattern=DesignPattern.MVVM, Name="MVVM (Spring Boot / JavaFX)", Architecture=ArchitectureType.Java, Description="Model-View-ViewModel para JavaFX o frontend desacoplado.", ImplementationNotes="ViewModel expone ObservableValue. Usar Property Binding de JavaFX o patrón Observer.", ScaffoldCommandsJson="[\"mkdir -p src/main/java/presentation/viewmodel src/main/java/presentation/view\"]" },
    };
}
