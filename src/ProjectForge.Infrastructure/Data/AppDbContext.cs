...
public class AppDbContext : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        // ── Seeders: uno por lenguaje / ecosistema ────────────────────────────
        DotNetSeeder.Seed(modelBuilder);      // C# / .NET
        PythonSeeder.Seed(modelBuilder);      // Python (FastAPI, Django, Flask)
        JavaScriptSeeder.Seed(modelBuilder);  // JavaScript / TypeScript
        PhpSeeder.Seed(modelBuilder);         // PHP (Laravel, Symfony)
        JavaSeeder.Seed(modelBuilder);        // Java (Spring Boot, Quarkus, Micronaut)
        TypeScriptSeeder.Seed(modelBuilder);  // TypeScript (NestTS, NextTS)
    }
}
